...
const Steps = ({ toogleTheme }) => {
	...
	const onSendNumberSubmit = async () => {
		...
		try {
			const responseData = await twilioService.startVerify(getCurrentNumber());
	...
	const onVerifySubmit = async (setError, currentCode) => {
		...
		try {
			const responseData = await twilioService.checkVerify(getCurrentNumber(), currentCode || code);
	...
	const goBack = async () => {
		...
		if (currentStepId === 2) {
			...
			try {
				const responseData = await twilioService.cancelVerify(getCurrentNumber());
